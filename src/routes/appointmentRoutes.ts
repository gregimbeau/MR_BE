...
router.post("/appointments", async (req, res) => {
  ...
  try {
    const newAppointment = await pool.query(
      "INSERT INTO appointments (title, type, location, vendor_id, buyer_id, start_time, end_time) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *",
...
router.put("/appointments/:id", async (req, res) => {
  ...
  try {
    const updateAppointment = await pool.query(
      "UPDATE appointments SET title = $1, type = $2, location = $3, vendor_id = $4, buyer_id = $5, start_time = $6, end_time = $7 WHERE id = $8 RETURNING *",
